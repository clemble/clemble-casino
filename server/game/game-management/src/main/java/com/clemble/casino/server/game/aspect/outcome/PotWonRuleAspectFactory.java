...
public class PotWonRuleAspectFactory implements PotGameAspectFactory<GamePotEndedEvent>{

    final private ServerPaymentTransactionService transactionService;

    public PotWonRuleAspectFactory(ServerPaymentTransactionService transactionService) {
    ...
    public GameAspect<GamePotEndedEvent> construct(PotGameConfiguration configuration, PotGameContext potContext) {
        ...
        if (configuration.getWonRule() != null)
            return new PotWonRuleAspect(configuration.getPrice().getCurrency(), transactionService);
