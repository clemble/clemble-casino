 ...
 */
public class SystemGameTimeoutEventListener implements SystemEventListener<SystemGameTimeoutEvent> {
    ...
    public SystemGameTimeoutEventListener(GameManagerFactoryFacade managerFactory) {
    ...
    public void onEvent(SystemGameTimeoutEvent event) {
        ...
        ClembleManager<GameManagementEvent, ? extends GameState> manager = managerFactory.get(event.getSessionKey());
        // Step 2. Extracting game context
        GameContext<GamePlayerContext> context = manager.getState().getContext();
