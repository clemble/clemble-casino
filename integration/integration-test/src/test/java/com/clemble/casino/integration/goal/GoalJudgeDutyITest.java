...
public class GoalJudgeDutyITest {
    ...
    public void testDuty() {
        ...
        Assert.assertEquals(B.goalOperations().goalDutyOperations().myDuties().size(), 0);
        ...
        B.goalOperations().goalInvitationOperations().reply(AtoBinvitation.getGoalKey(), AtoBinvitation.cloneWithStatus(GoalJudgeInvitationStatus.accepted));
        // Step 5. Checking duty appeared
        AsyncCompletionUtils.check(new Check() {
            @Override
            public boolean check() {
                return B.goalOperations().goalDutyOperations().myDuties().size() == 1;
            }
        }, 30_000, 1_000);
        // Step 6. Checking duty operations
        Assert.assertEquals(B.goalOperations().goalDutyOperations().myDuties().iterator().next(), GoalJudgeDuty.fromInvitation(AtoBinvitation));
    }
