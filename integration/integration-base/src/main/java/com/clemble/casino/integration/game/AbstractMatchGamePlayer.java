...
abstract public class AbstractMatchGamePlayer<State extends GameState> implements MatchGamePlayer<State>, EventListener<GameSessionAwareEvent>, Closeable {
    ...
    final private Collection<MatchGamePlayer<State>> dependents = new LinkedBlockingQueue<MatchGamePlayer<State>>();
    ...
    public AbstractMatchGamePlayer(final ClembleCasinoOperations player, GameConstruction construction) {
    ...
    public void perform(GameAction gameAction) {
        ...
        for (MatchGamePlayer<State> player : dependents)
    ...
    public void addDependent(GamePlayer player) {
        if(!(player instanceof MatchGamePlayer))
            throw new IllegalArgumentException();
        if (player != null && player != this) {
            dependents.add((MatchGamePlayer<State>) player);
        }
    }
