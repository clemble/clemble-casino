...
public class TicTacToeProcessor implements GameProcessor<TicTacToeState> {
    ...
    @Override
    public GameServerEvent<TicTacToeState> process(final GameSession<TicTacToeState> session, ClientEvent clientEvent) {
        TicTacToeState state = session.getState();
        // Step 1. Processing Select cell move
        if (state.getOutcome() != null) {
            return new GameEndedEvent<TicTacToeState>(state).setOutcome(state.getOutcome());
        } else if (clientEvent instanceof SelectCellEvent) {
            return processSelectCellEvent(state, (SelectCellEvent) clientEvent);
        } else if (clientEvent instanceof BetEvent) {
            return processBetOnCellEvent(state, (BetEvent) clientEvent);
    ...
    private GameServerEvent<TicTacToeState> processSurrenderEvent(final TicTacToeState state, final SurrenderEvent surrenderEvent) {
        ...
        if (opponents.size() == 0 || state.getVersion() == 1) {
            ...
            return new GameEndedEvent<TicTacToeState>().setOutcome(new NoOutcome()).setState(state);
        } else {
            ...
            return new GameEndedEvent<TicTacToeState>(state).setOutcome(new PlayerWonOutcome(winner));
        }
    }
