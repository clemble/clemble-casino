package com.clemble.casino.android.service;
...
public class AndroidRestService implements RestClientService {
    ...
    private AndroidRestService(ServerRegistry baseUrl, RestTemplate restTemplate, PlayerSecurityClientService<HttpEntity<?>> securityClientService) {
        this.securityClientService = securityClientService;
        this.restTemplate = restTemplate;
        this.baseUrl = baseUrl;
    }

    public AndroidRestService(ServerRegistry baseUrl, ObjectMapper objectMapper, PlayerSecurityClientService<HttpEntity<?>> securityClientService) {
    ...
    public <T> T getForEntity(CharSequence url, Class<T> responseType, Object... urlVariables) {
        String fullURL = baseUrl.findBase() + url;
    ...
    public <T> T putForEntity(CharSequence url, Object requestObject, Class<T> responseType, Object... urlVariables) {
        ...
        return restTemplate.exchange(baseUrl.findBase() + url, HttpMethod.PUT, request, responseType, urlVariables).getBody();
    }

    @Override
    public <T> T postForEntity(CharSequence url, Object requestObject, Class<T> responseType, Object... urlVariables) {
        HttpEntity<?> request = securityClientService.signUpdate(requestObject);
        return restTemplate.exchange(baseUrl.findBase() + url, HttpMethod.POST, request, responseType, urlVariables).getBody();
    }
    ...
    @Override
    public RestClientService construct(ServerRegistry baseUrl) {
